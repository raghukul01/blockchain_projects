...
bool txn_t::validate() 
{
    ...
    sha256((const uint8_t*) public_key.data(), public_key.size(), h1);

    uint8_t* source_addr_val = source_addr.data();

    for(int i = 0; i < SHA256_DIGEST_LENGTH; i++) 
        if(source_addr_val[i] != h1[i])
            return false;

    // 2. SHA256(public_key, source_addr, dest_addr, change_addr, amount) = tx_hash
    SHA256_CTX sha256;
    SHA256_Init(&sha256);
    SHA256_Update(&sha256, public_key.data(), public_key.size());
    SHA256_Update(&sha256, source_addr.data(), source_addr.size());
    ...
    uint8_t* tx_hash_val = tx_hash.data();

    for(int i = 0; i < SHA256_DIGEST_LENGTH; i++) 
        if(tx_hash_val[i] != h2[i])
            return false;
